...
- **Removed AI predictive pathing feature**: Replaced with a "danger score" algorithm that calculates the danger between a weather event and the user's current location.
- **Added custom markers**: Users can now add custom markers for safehouses, resource points, and dangers.
- **Added saved locations**: Users can save important locations for easy tracking.
- **Updated Actors Description**: Removed references to predictive weather pathing and updated descriptions to reflect new features.
- **Updated Functional Requirements**: Removed predictive weather pathing and added requirements for custom markers, danger scores, and saved locations.
- **Updated Main Project Complexity Design**: Replaced wildfire path prediction with danger score calculation.
- **Updated Interfaces**: Iterated on all interfaces specified for each component in order to better represent component functionality  
...
1. **[Basic User]**: Basic Users can interact with the map to get information about the weather conditions in their area. These users can also view weather event markers, read comments, and become Authenticated Users.
2. **[Authenticated User]**: Authenticated Users can interact with the app with all the same privileges as an Basic User, but can also enable push notifications, create comments, add custom markers (e.g., safehouses, resource points, dangers), and save locations.
...
1. **Create Account**
      - **Description**: Basic Users can create an account using their email or social media accounts (e.g., Google, Facebook). Once authenticated, they become Authenticated Users.
      - **Primary actor(s)**: Basic Users
      - **Main success scenario**:
         ...
         5. User is redirected to the app's main interface as an Authenticated User.
      - **Failure scenario(s)**:
...
2. **[Select Weather Event]**:
   - **Description**:All Users (Basic and Authenticated) can click on weather event markers on the map to view detailed information about the event, including the danger score.
   - **Primary actor(s)**: Basic Users, Authenticated Users
   - **Main success scenario**:
...
3. **[Add Custom Marker]**:
   - **Description**: Authenticated Users can add custom markers to the map for safehouses, resource points, or dangers. These markers are visible to all users.
...
4. **[Add Comment]**:
   - **Description**:Authenticated Users can add comments to weather events to share important information about the public events. 
   - **Primary actor(s)**: Authenticated Users
   - **Main success scenario**:
...
5. **[Save Location to Favorites]**:
   - **Description**: Authenticated Users can save specific locations on the map to their "Favorites" list for easy access later. "Favorites" can also be used to receive notifications in case of danger.
   - **Primary actor(s)**: Authenticated Users
   - **Main success scenario**:
...
6. **[Enable Warning Notifications]**:
   - **Description**: Authenticated Users can enable push notifications to receive alerts about extreme weather events in their area.
   - **Primary actor(s)**: Authenticated Users
   - **Main success scenario**:
...
1. **Authentication**
   ...
   - **Purpose**: The authentication component authenticates and setup a user with their Google accounts allowing them to become an “Authenticated User” actor in the app.
...
4. **Weather**
   ...
   - **Interfaces**:
     ...
     2. void notifyUser (String eventId, String userId):
