...
app.post('/user_marker', async (req, res) => {
  ...
  if (!type || !latitude || !longitude || !description) {
    ...
    return res.status(400).json({ error: 'Missing type, latitude, longitude, or description in request body'+type+latitude+longitude+description });
  }
...
app.post('/comment/:id', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Error appending comment '+error+' '+params });
  }
});
...
app.post('/user', async (req, res) => {
  ...
  if (!name || !location || !account_type || !email || !regToken || notifications == null) {
    console.log("Bad params", req.body);
    console.log(`name: ${name}, location: ${location}, account_type: ${account_type}, email: ${email}, regToken: ${regToken}, notifications: ${notifications}`)
